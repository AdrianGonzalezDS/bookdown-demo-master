...
#Prepare datos
victimasdelvictidel <- victimasdelito %>%
  select(infodelito2,prensa,victi_deli2, tipo_delito) %>%
  filter(infodelito2 == "Si"&
           tipo_delito == "Homicidio intencional")

victimasdelvictidel

victimasdelvictideli_sel <- victimasdelito %>%
  select(infodelito2,prensa,victi_deli2, tipo_delito) %>%
  filter(infodelito2 == "Si"&
           tipo_delito == "Homicidio intencional"&
           !victi_deli2 %in% c(NA, "NA"))

victimasdelvictideli_sel

prensa_victimasdelvictideli_sel <- length(unique(victimasdelvictideli_sel[["prensa"]]))
prensa_victimasdelvictideli_sel

victimasdelvictideli <- data.frame(victimasdelvictideli_sel$victi_deli2)
victimasdelvictideli
...
# Data transformation
victimasdelvictideli_porcent <- victimasdelvictideli %>%
  group_by(victimasdelvictideli_sel.victi_deli2) %>% # Variable a ser transformada
  count() %>%
  ungroup() %>%
...
victimasdelvictideli_porcent

victimasdelvictideli_donagraf <- ggplot(data = victimasdelvictideli_porcent,
                                         aes(x = 2, y = perc,
                                             fill = victimasdelvictideli_sel.victi_deli2))+
  ...
  labs(caption = stringr::str_glue("Fuente: Observatorio de prensa OVV  \nn = {nrow(victimasdelvictidel)} ({sum(is.na(victimasdelvictidel$victi_deli2)| victimasdelvictidel$victi_deli2 == 'NA')} casos perdidos por información faltante) en {prensa_victimasdelvictideli_sel} medios de prensa consultados \nPeríodo de recolección de información: {format(startdate, '%d %b')}-{format(enddate, '%d %b %Y')}"))+
  geom_text_repel(aes(label = porcentaje, x = 1.95),
                  position = position_stack(vjust = 0.5),
                  color = c("white", "white", "white"))

victimasdelvictideli_donagraf
