...
sucesoshi_tipoarma_sel <- sucesos %>%
  select(infodelito2, prensa, tipo_delito,arma_delito1) %>%
  filter(infodelito2 == "Si"&
           tipo_delito == "Homicidio intencional"& !infodelito2 %in% c(NA, "NA") &
           !arma_delito1 %in% c(NA, "NA"))

sucesoshi_tipoarma_sel

prensa_sucesoshi_tipoarma_sel <- length(unique(sucesoshi_tipoarma_sel[["prensa"]]))
prensa_sucesoshi_tipoarma_sel

sucesoshi_tipoarma_sel$prensa<- NULL


#Poniendo acentos en la leyenda
sucesoshi_tipoarma_sel$arma_delito1 <- str_replace_all(sucesoshi_tipoarma_sel$arma_delito1,
                                                          "[^[:alnum:]]"," ")
...
# Data transformation
porcent_grup_sucesoshitipoarma <- sucesoshi_tipoarma_sel %>%
...
porcent_grup_sucesoshitipoarma



#graficando

sucesoshitipoarma_barras <- ggplot(porcent_grup_sucesoshitipoarma,
  ...
  geom_label(aes(x = arma_delito1, y = freq, label = porcentaje),
             ...
             family="Helvetica",
             size = 3)+
  labs(caption = stringr::str_glue("Fuente: Observatorio de prensa OVV  \nn = {nrow(sucesoshi_tipoarma)} ({sum(is.na(sucesoshi_tipoarma$arma_delito1)| sucesoshi_tipoarma$arma_delito1 == 'NA'|is.na(sucesoshi_tipoarma$infodelito2)|sucesoshi_tipoarma$infodelito2 == 'NA')} casos perdidos por información faltante) en {prensa_sucesoshi_tipoarma_sel} medios de prensa consultados \nPeríodo de recolección de información: {format(startdate, '%d %b')}-{format(enddate, '%d %b %Y')}"))+
  xlab("") +
  ylab("Número de sucesos")

sucesoshitipoarma_barras
