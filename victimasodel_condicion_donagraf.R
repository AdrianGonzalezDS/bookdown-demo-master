...
victimasodelcondicion_donagraf <- ggplot(data = victimasodelcondicion_porcent,
  ...
  labs(caption = stringr::str_glue("Fuente: Observatorio de prensa OVV  \nn = {nrow(victimasodelcondi)} ({sum(is.na(victimasodelcondi$condicion_victima_2)| victimasodelcondi$condicion_victima_2 == 'NA')} casos perdidos por información faltante) en {prensa_victimasodelcondicion_sel} medios de prensa consultados \nPeróodo de recolección de información: {format(startdate, '%d %b')}-{format(enddate, '%d %b %Y')}"))+
