...
names(tabla_ocupacion)[names(tabla_ocupacion) ==
                         "victimasodelocupacion_sel.ocupacion_victima_2"] <- "Ocupación"
names(tabla_ocupacion)[names(tabla_ocupacion) ==
                         "freq"] <- "Víctimas"
...
victimasodelocupacion_donagraf <- ggplot(data = victimasodelocupacion_porcent,
  ...
  labs(caption = stringr::str_glue("Fuente: Observatorio de prensa OVV  \nn = {nrow(victimasodelocupa)} ({sum(is.na(victimasodelocupa$ocupacion_victima_2)| victimasodelocupa$ocupacion_victima_2 == 'NA')} casos perdidos por información faltante) en {prensa_victimasodelocupacion_sel} medios de prensa consultados \nPeríodo de recolección de información: {format(startdate, '%d %b')}-{format(enddate, '%d %b %Y')}"))+
